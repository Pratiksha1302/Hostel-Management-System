...
$roomno = isset($_POST['room']) ? $_POST['room'] : '';
$seater = isset($_POST['seater']) ? $_POST['seater'] : '';
$feespm = isset($_POST['fpm']) ? $_POST['fpm'] : '';
$foodstatus = isset($_POST['foodstatus']) ? $_POST['foodstatus'] : '';
$stayfrom = isset($_POST['stayf']) ? $_POST['stayf'] : '';
...
$emcntno = isset($_POST['econtact']) ? $_POST['econtact'] : '';
$gurname = isset($_POST['gname']) ? $_POST['gname'] : '';
$gurrelation = isset($_POST['grelation']) ? $_POST['grelation'] : '';
$gurcntno = isset($_POST['gcontact']) ? $_POST['gcontact'] : '';
$caddress = isset($_POST['address']) ? $_POST['address'] : '';
$ccity = isset($_POST['city']) ? $_POST['city'] : '';
$cstate = isset($_POST['state']) ? $_POST['state'] : '';
$cpincode = isset($_POST['pincode']) ? $_POST['pincode'] : '';
...
if($stmt->execute()) {
	...
	// Registration failed
	echo "<script>alert('Error: Unable to register student');</script>";
}
...
<div class="col-sm-8">
<input type="radio" value="0" name="foodstatus" id="without_food" checked="checked"> Without Food
<input type="radio" value="1" name="foodstatus" id="with_food" onchange="updateTotal()"> With Food(Rs 2000.00 Per Month Extra)
</div>
</div>	
...
<script type="text/javascript">
    function updateTotal() {
        var fpm = parseFloat(document.getElementById("fpm").value); // Get the base fees per month
        var foodStatus = document.querySelector('input[name="foodstatus"]:checked').value; // Get the selected food status
        var total = fpm; // Initialize total with base fees

        // If food status is "With Food", add Rs 2000 to the total
        if (foodStatus === "1") {
            total += 2000;
        }
...
document.getElementById('uploadfile').addEventListener('change', function(event) {
    ...
    reader.onload = function() {
        ...
        document.getElementById('display-uploaded-image').style.display = 'block';
    };
...
<!-- Display success prompt after uploading image -->
<script>
